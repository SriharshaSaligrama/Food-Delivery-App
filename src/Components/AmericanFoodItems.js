...
export const AmericanFoodItems = props => {
    const {foodItems}=useSelector(state=>state)
    const {cartItems}=useSelector(state=>state)
    const AmericanFood=foodItems.filter(item=>item.American===true)
    const AmericanVegFood=AmericanFood.filter(item=>item.isVeg)
    ...
    const [searchedItems, setSearchedItems]=useState([...AmericanFood])
    ...
    const handleChange= e => {
        ...
        const searchedFoodItems=vegChecked?AmericanVegFood.filter(item=>item.name.includes(e.target.value.toLowerCase())):AmericanFood.filter(item=>item.name.includes(e.target.value.toLowerCase()))
        setSearchedItems(searchedFoodItems)
    }
    
    const handleCheck= e=>{
        setVegChecked(!vegChecked)
        setSearchedItems(e.target.checked?[...AmericanVegFood]:[...AmericanFood])
        setSearchText('')
    }
    ...
    return (
        <div>
            <Navbar bg="light" expand="lg">
                <Container fluid>
                    <Navbar.Brand href="#">American ({searchedItems.length}/{vegChecked?AmericanVegFood.length:AmericanFood.length})</Navbar.Brand>
